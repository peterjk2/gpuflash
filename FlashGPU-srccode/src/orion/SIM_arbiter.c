...
static double SIM_matrix_arbiter_grant_cap(u_int req_width) {
  ...
  return (req_width * SIM_draincap(WdecNORn, NCH, 1) +
          SIM_draincap(WdecNORp, PCH, req_width));
}
...
int SIM_arbiter_init(SIM_arbiter_t *arb, int arbiter_model, int ff_model,
                     u_int req_width, double length, SIM_array_info_t *info) {
  if ((arb->model = arbiter_model) && arbiter_model < ARBITER_MAX_MODEL) {
    ...
    switch (arbiter_model) {
      case RR_ARBITER:
        ...
        I_static = 0;
        /* NOR */
        I_static += (6 * arb->req_width *
                     ((WdecNORp * NOR2_TAB[0] +
                       WdecNORn * (NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3])) /
                      4));

        /* inverter */
        I_static += 2 * arb->req_width *
                    ((Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) / 2);
        /* DFF */
      ...
      case MATRIX_ARBITER:
        arb->e_chg_req =
            SIM_matrix_arbiter_req_cap(req_width, length) / 2 * EnergyFactor;
        /* 2 grant signals switch together, so no 1/2 */
        arb->e_chg_grant =
            SIM_matrix_arbiter_grant_cap(req_width) * EnergyFactor;
        arb->e_chg_mint = SIM_matrix_arbiter_int_cap() / 2 * EnergyFactor;
        arb->e_chg_carry = arb->e_chg_carry_in = 0;

        if (SIM_fpfp_init(&arb->pri_ff, ff_model,
                          SIM_matrix_arbiter_pri_cap(req_width)))
          return -1;
        /*arbiter static power */
        I_static = 0;
        /* NOR */
        I_static += ((2 * arb->req_width - 1) * arb->req_width) *
                    ((WdecNORp * NOR2_TAB[0] +
                      WdecNORn * (NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3])) /
                     4);
        /* inverter */
        I_static += arb->req_width *
                    ((Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) / 2);
        /* DFF */
        I_static +=
            (arb->req_width * (arb->req_width - 1) / 2) * (Wdff * DFF_TAB[0]);
        arb->I_static = I_static;
...
int SIM_arbiter_record(SIM_arbiter_t *arb, LIB_Type_max_uint new_req,
                       LIB_Type_max_uint old_req, u_int new_grant,
                       u_int old_grant) {
  switch (arb->model) {
    case MATRIX_ARBITER:
...
double SIM_arbiter_report(SIM_arbiter_t *arb) {
  switch (arb->model) {
    case MATRIX_ARBITER:
      return (arb->n_chg_req * arb->e_chg_req +
              arb->n_chg_grant * arb->e_chg_grant +
    ...
    case RR_ARBITER:
      return (arb->n_chg_req * arb->e_chg_req +
              arb->n_chg_grant * arb->e_chg_grant +
              arb->n_chg_carry * arb->e_chg_carry +
              arb->n_chg_carry_in * arb->e_chg_carry_in +
              arb->pri_ff.n_switch * arb->pri_ff.e_switch +
              arb->pri_ff.n_keep_1 * arb->pri_ff.e_keep_1 +
              arb->pri_ff.n_keep_0 * arb->pri_ff.e_keep_0 +
              arb->pri_ff.n_clock * arb->pri_ff.e_clock);
...
double SIM_arbiter_stat_energy(SIM_arbiter_t *arb, SIM_array_info_t *info,
                               double n_req, int print_depth, char *path,
                               int max_avg) {
  ...
  switch (arb->model) {
    case RR_ARBITER:
      ...
      Eatomic =
          arb->e_chg_carry * arb->req_width * (max_avg ? 1 : 0.5) * n_grant;
      SIM_print_stat_energy(SIM_strcat(path, "carry"), Eatomic, next_depth);
      SIM_res_path(path, path_len);
      Eavg += Eatomic;

      Eatomic = arb->e_chg_carry_in *
                (arb->req_width * (max_avg ? 1 : 0.5) - 1) * n_grant;
      SIM_print_stat_energy(SIM_strcat(path, "internal carry"), Eatomic,
                            next_depth);
      SIM_res_path(path, path_len);
      Eavg += Eatomic;
      ...
      SIM_print_stat_energy(SIM_strcat(path, "switch"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;

      Eatomic = arb->pri_ff.e_keep_0 * (arb->req_width - 2 * n_grant);
      SIM_print_stat_energy(SIM_strcat(path, "keep 0"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;

      Eatomic = arb->pri_ff.e_clock * arb->req_width;
      SIM_print_stat_energy(SIM_strcat(path, "clock"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;
    ...
    case MATRIX_ARBITER:
      ...
      if (arb->pri_ff.e_keep_0 >= arb->pri_ff.e_keep_1 || !max_avg) {
        Eatomic = arb->pri_ff.e_keep_0 * (total_pri - n_chg_pri * n_grant) *
                  (max_avg ? 1 : 0.5);
        SIM_print_stat_energy(SIM_strcat(path, "keep 0"), Eatomic,
                              next_next_depth);
        SIM_res_path(path, next_path_len);
      ...
      if (arb->pri_ff.e_keep_0 < arb->pri_ff.e_keep_1 || !max_avg) {
        Eatomic = arb->pri_ff.e_keep_1 * (total_pri - n_chg_pri * n_grant) *
                  (max_avg ? 1 : 0.5);
        SIM_print_stat_energy(SIM_strcat(path, "keep 1"), Eatomic,
                              next_next_depth);
        SIM_res_path(path, next_path_len);
        Estruct += Eatomic;
      }
      ...
      if (max_avg)
        /* min(p,n/2)(n-1) + 2(n-1) */
        Eatomic = arb->e_chg_mint * (MIN(n_req, arb->req_width * 0.5) + 2) *
                  (arb->req_width - 1);
      else
        /* p(n-1)/2 + (n-1)/2 */
    ...
    case QUEUE_ARBITER:
      ...
      Eavg =
          SIM_array_stat_energy(info, &arb->queue, n_req, n_grant, next_depth,
                                SIM_strcat(path, "queue"), max_avg);
      SIM_res_path(path, path_len);
      break;

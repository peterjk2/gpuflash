 ...
 * 	  (3) address decomposition does not consider column selector and
 * sub-array now
 *
 * NOTES: (1) power stats function should not be called before its functional
 * counterpart except data_write
 *
 * TODO:  (2) How to deal with RF's like AH/AL/AX?  Now I assume output with 0
 * paddings. (1) make _dec handle both row dec and col dec, no wordline (3) move
 * wordline stats to _read and _write
 */
...
int SIM_array_port_state_init(SIM_array_info_t *info,
                              SIM_array_port_state_t *port) {
...
int SIM_array_set_state_init(SIM_array_info_t *info,
                             SIM_array_set_state_t *set) {
  set->entry = NULL;
  set->entry_set = NULL;
...
int SIM_array_dec(SIM_array_info_t *info, SIM_array_t *arr,
                  SIM_array_port_state_t *port, LIB_Type_max_uint row_addr,
                  int rw) {
  if (!IS_FULLY_ASSOC(info)) {
...
int SIM_array_data_read(SIM_array_info_t *info, SIM_array_t *arr,
                        LIB_Type_max_uint data) {
  if (info->data_end == 1) {
    SIM_array_bitline_record(&arr->data_bitline, SIM_ARRAY_READ,
                             info->eff_data_cols, 0, data);

    return 0;
  }
  else if (!IS_FULLY_ASSOC(info)) {
    SIM_array_bitline_record(&arr->data_bitline, SIM_ARRAY_READ,
                             info->eff_data_cols, 0, 0);
...
int SIM_array_data_write(SIM_array_info_t *info, SIM_array_t *arr,
                         SIM_array_set_state_t *set, u_int n_item,
                         u_char *data_line, u_char *old_data,
                         u_char *new_data) {
  u_int i;
  ...
  if (IS_FULLY_ASSOC(info)) {
    ...
    for (i = 0; i < n_item; i++) {
      SIM_array_bitline_record(&arr->data_bitline, SIM_ARRAY_WRITE, 8,
    ...
    /* there is some subtlety here: write width may not be as wide as block
     * size, bitlines not written are actually read, but column selector should
     * be off, so read energy per bitline is the same as write energy per
     * bitline */
    SIM_array_bitline_record(&arr->data_bitline, SIM_ARRAY_WRITE,
                             info->eff_data_cols, 0, 0);

    /* write in all sub-arrays if direct-mapped, which implies 1 cycle write
     * latency, in those sub-arrays wordlines are not driven, so only n items
     * columns switch */
    if (IS_DIRECT_MAP(info) && info->data_ndbl > 1)
      SIM_array_bitline_record(&arr->data_bitline, SIM_ARRAY_WRITE,
                               n_item * 8 * (info->data_ndbl - 1), 0, 0);
  }
  else { /* separate R/W bitlines */
    /* same arguments as in the previous case apply here, except that when we
     * say read_energy = write_energy, we omit the energy of write driver gate
     * cap */
    for (i = 0; i < n_item; i++) {
      SIM_array_bitline_record(&arr->data_bitline, SIM_ARRAY_WRITE, 8,
                               data_line[i], new_data[i]);
...
/* set only used by fully-associative array */
int SIM_array_tag_read(SIM_array_info_t *info, SIM_array_t *arr,
                       SIM_array_set_state_t *set) {
  if (IS_FULLY_ASSOC(info)) {
  ...
  SIM_array_bitline_record(&arr->tag_bitline, SIM_ARRAY_READ,
                           info->eff_tag_cols, 0, 0);
...
int SIM_array_tag_update(SIM_array_info_t *info, SIM_array_t *arr,
                         SIM_array_port_state_t *port,
                         SIM_array_set_state_t *set) {
  u_int i;
  ...
  if (IS_FULLY_ASSOC(info)) {
    if (set->entry && curr_tag != set->tag_bak) {
      ...
      SIM_array_bitline_record(&arr->tag_bitline, SIM_ARRAY_WRITE,
                               info->eff_tag_cols, port->tag_line, curr_tag);
      /* update state */
  ...
  if (HAVE_USE_BIT(info))
    for (i = 0; i < info->assoc; i++)
      SIM_array_mem_record(tag_attach_mem, set->use_bak[i],
                           (*info->get_set_use_bit)(set->entry_set, i),
                           info->use_bit_width);

  if (set->entry) {
    SIM_array_mem_record(tag_attach_mem, set->valid_bak,
                         (*info->get_entry_valid_bit)(set->entry),
                         info->valid_bit_width);
    SIM_array_mem_record(&arr->tag_mem, set->tag_bak, curr_tag,
                         info->tag_addr_width);

    if (IS_WRITE_BACK(info))
      SIM_array_mem_record(tag_attach_mem, set->dirty_bak,
                           (*info->get_entry_dirty_bit)(set->entry), 1);
  }

  return 0;
}

/* record tag compare activity (including tag comparator, column decoder and
 * multiplexor) */
/* NOTE: this function may be called twice during ONE array operation, remember
 * to update states at the end so that call to *_record won't add erroneous
 * extra energy */
/* only used by non-RF array */
int SIM_array_tag_compare(SIM_array_info_t *info, SIM_array_t *arr,
                          SIM_array_port_state_t *port,
                          LIB_Type_max_uint tag_input,
  ...
  for (i = 0; i < info->assoc; i++) {
    ...
    if (SIM_array_comp_local_record(&arr->comp, 0,
                                    (*info->get_set_tag)(set->entry_set, i),
...
int SIM_array_output(SIM_array_info_t *info, SIM_array_t *arr, u_int data_size,
                     u_int length, void *data_out, void *data_all) {
  u_int i, j;
  ...
  for (i = 0; i < length; i++) {
    switch (data_size) {
      case 1:
        ...
        break;
      case 2:
        SIM_array_outdrv_global_record(&arr->outdrv,
                                       ((u_int16_t *)data_out)[i]);
        break;
      case 4:
        SIM_array_outdrv_global_record(&arr->outdrv,
                                       ((u_int32_t *)data_out)[i]);
        break;
      case 8:
        SIM_array_outdrv_global_record(&arr->outdrv,
                                       ((u_int64_t *)data_out)[i]);
        break;
      default:
        printf("error\n"); /* some error handler */
    }
  ...
  if (!IS_FULLY_ASSOC(info)) {
    for (i = 0; i < info->assoc; i++)
      for (j = 0; j < info->n_item; j++)
        ...
        switch (data_size) {
          case 1:
            SIM_array_outdrv_local_record(&arr->outdrv, 0,
                                          ((u_int8_t **)data_all)[i][j],
                                          SIM_ARRAY_RECOVER);
            break;
          case 2:
            SIM_array_outdrv_local_record(&arr->outdrv, 0,
                                          ((u_int16_t **)data_all)[i][j],
                                          SIM_ARRAY_RECOVER);
            break;
          case 4:
            SIM_array_outdrv_local_record(&arr->outdrv, 0,
                                          ((u_int32_t **)data_all)[i][j],
                                          SIM_ARRAY_RECOVER);
            break;
          case 8:
            SIM_array_outdrv_local_record(&arr->outdrv, 0,
                                          ((u_int64_t **)data_all)[i][j],
                                          SIM_ARRAY_RECOVER);
            break;
          default:
            printf("error\n"); /* some error handler */
...
double SIM_array_stat_energy(SIM_array_info_t *info, SIM_array_t *arr,
                             double n_read, double n_write, int print_depth,
                             char *path, int max_avg) {
  ...
  if (info->arr_buf_type == SRAM) {
    ...
    if (info->row_dec_model) {
      ...
      Eatomic = arr->row_dec.e_chg_addr * arr->row_dec.n_bits *
                (max_avg ? 1 : 0.5) * (n_read + n_write);
      SIM_print_stat_energy(SIM_strcat(path, "input"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;
      Eatomic = arr->row_dec.e_chg_output * (n_read + n_write);
      SIM_print_stat_energy(SIM_strcat(path, "output"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;
      ...
      Eatomic =
          arr->row_dec.e_chg_l1 * arr->row_dec.n_in_2nd * (n_read + n_write);
      SIM_print_stat_energy(SIM_strcat(path, "internal node"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;
    ...
    if (arr->data_bitline.end == 2) {
      ...
      /* dirty hack */
      if (share_flag) {
        Eatomic += arr->data_bitline.e_col_read *
      ...
      Eatomic = arr->data_bitline.e_col_read * info->eff_data_cols *
                (max_avg ? 1 : 0.5) * n_read;
      /* dirty hack */
      if (share_flag) {
        /* assume no multicasting, cnt is always 0 */
        Eatomic += arr->data_bitline.e_col_read * (NP_width + NC_width) *
                   (max_avg ? 1 : 0.5) * n_read;
        /* read free list */
        Eatomic += arr->data_bitline.e_col_read * (NP_width + NC_width) *
    ...
    Eatomic = arr->data_bitline.e_col_write * info->data_width *
              (max_avg ? 1 : 0.5) * n_write;
    /* dirty hack */
    if (share_flag) {
      /* current NP and NC */
      Eatomic += arr->data_bitline.e_col_write * (NP_width + NC_width) *
                 (max_avg ? 1 : 0.5) * n_write;
      /* previous NP or NC */
      Eatomic += arr->data_bitline.e_col_write * NP_width *
                 (max_avg ? 1 : 0.5) * n_write;
      /* update free list */
      Eatomic += arr->data_bitline.e_col_write * NC_width *
                 (max_avg ? 1 : 0.5) * n_read;
    }
    SIM_print_stat_energy(SIM_strcat(path, "write"), Eatomic, next_next_depth);
    SIM_res_path(path, next_path_len);
    ...
    /* dirty hack */
    if (share_flag) {
      Eatomic += arr->data_bitline_pre.e_charge *
                 (NP_width + NC_width + cnt_width) * n_read;
      /* read free list */
      Eatomic += arr->data_bitline_pre.e_charge *
                 (NP_width + NC_width + cnt_width) * n_write;
    }
    SIM_print_stat_energy(SIM_strcat(path, "precharge"), Eatomic,
                          next_next_depth);
    SIM_res_path(path, next_path_len);
    Estruct += Eatomic;
    ...
    Eatomic = arr->data_mem.e_switch * info->data_width * (max_avg ? 1 : 0.5) *
              n_write;
    /* dirty hack */
    if (share_flag) {
      /* current NP and NC */
      Eatomic += arr->data_mem.e_switch * (NP_width + NC_width) *
                 (max_avg ? 1 : 0.5) * n_write;
      /* previous NP or NC */
      Eatomic +=
          arr->data_mem.e_switch * NP_width * (max_avg ? 1 : 0.5) * n_write;
      /* update free list */
      Eatomic +=
          arr->data_mem.e_switch * NC_width * (max_avg ? 1 : 0.5) * n_read;
    }
    Estruct += Eatomic;
    ...
    if (info->data_end == 2) {
      ...
      if (share_flag) {
        Eatomic +=
            arr->data_amp.e_access * (NP_width + NC_width + cnt_width) * n_read;
        /* read free list */
        Eatomic += arr->data_amp.e_access * (NP_width + NC_width + cnt_width) *
                   n_write;
      }
      Estruct += Eatomic;

      SIM_print_stat_energy(SIM_strcat(path, "sense amplifier"), Estruct,
                            next_depth);
      SIM_res_path(path, path_len);
      Eavg += Estruct;
    }
    ...
    if (info->outdrv_model) {
      ...
      SIM_print_stat_energy(SIM_strcat(path, "enable"), Eatomic,
                            next_next_depth);
      SIM_res_path(path, next_path_len);
      Estruct += Eatomic;

      /* same switch probability as bitlines */
      Eatomic = arr->outdrv.e_chg_data * arr->outdrv.item_width *
                (max_avg ? 1 : 0.5) * info->n_item * info->assoc * n_read;
      ...
      if (arr->outdrv.e_out_1 >= arr->outdrv.e_out_0 || !max_avg) {
        Eatomic = arr->outdrv.e_out_1 * arr->outdrv.item_width *
                  (max_avg ? 1 : 0.5) * n_read;
        SIM_print_stat_energy(SIM_strcat(path, "output 1"), Eatomic,
                              next_next_depth);
        SIM_res_path(path, next_path_len);
        Estruct += Eatomic;
      }
      ...
      if (arr->outdrv.e_out_1 < arr->outdrv.e_out_0 || !max_avg) {
        Eatomic = arr->outdrv.e_out_0 * arr->outdrv.item_width *
                  (max_avg ? 1 : 0.5) * n_read;
        SIM_print_stat_energy(SIM_strcat(path, "output 0"), Eatomic,
                              next_next_depth);
        SIM_res_path(path, next_path_len);
        Estruct += Eatomic;
      }
    ...
    SIM_print_stat_energy(SIM_strcat(path, "static energy"), Estatic,
                          next_depth);
    SIM_res_path(path, path_len);
    ...
    avg_write =
        arr->ff.e_switch * arr->ff.n_switch + arr->ff.e_clock * arr->ff.n_clock;

    /* for each read operation, the energy consists of one read operation and n
     * write operateion. n means there is n flits in the buffer before read
     * operation. assume n is info->n_entry * 0.25.
     */
    if (info->n_set > 1) {
    ...
    SIM_print_stat_energy(SIM_strcat(path, "write energy"), Eatomic,
                          next_depth);
    SIM_res_path(path, path_len);
    Estruct += Eatomic;

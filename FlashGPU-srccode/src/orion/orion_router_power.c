...
int main(int argc, char **argv) {
  ...
  if (optind >= argc) {
    fprintf(
        stderr,
        "orion_router_power: [-pm] [-d print_depth] [-l load] <router_name>\n");
    return 1;
  }
  else {
    name = argv[optind];
  ...
  SIM_router_stat_energy(&GLOB(router_info), &GLOB(router_power), print_depth,
                         name, max_flag, load, plot_flag, PARM(Freq));

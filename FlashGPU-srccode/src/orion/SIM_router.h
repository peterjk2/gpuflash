...
extern int SIM_router_init(SIM_router_info_t *info,
                           SIM_router_power_t *router_power,
                           SIM_router_area_t *router_area);

extern int SIM_buf_power_data_read(SIM_array_info_t *info, SIM_array_t *arr,
                                   LIB_Type_max_uint data);
extern int SIM_buf_power_data_write(SIM_array_info_t *info, SIM_array_t *arr,
                                    u_char *data_line, u_char *old_data,
                                    u_char *new_data);

extern int SIM_router_power_init(SIM_router_info_t *info,
                                 SIM_router_power_t *router);
extern int SIM_router_power_report(SIM_router_info_t *info,
                                   SIM_router_power_t *router);
extern double SIM_router_stat_energy(SIM_router_info_t *info,
                                     SIM_router_power_t *router,
                                     int print_depth, char *path, int max_avg,
                                     double e_fin, int plot_flag, double freq);

extern int SIM_router_area_init(SIM_router_info_t *info,
                                SIM_router_area_t *router_area);
extern double SIM_router_area(SIM_router_area_t *router_area);

extern int SIM_crossbar_record(SIM_crossbar_t *xb, int io,
                               LIB_Type_max_uint new_data,
                               LIB_Type_max_uint old_data, u_int new_port,
                               u_int old_port);
extern int SIM_arbiter_record(SIM_arbiter_t *arb, LIB_Type_max_uint new_req,

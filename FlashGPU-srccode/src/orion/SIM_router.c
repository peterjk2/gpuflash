...
int SIM_router_init(SIM_router_info_t *info, SIM_router_power_t *router_power,
  ...
  info->n_total_in =
      PARM(in_port) + PARM(cache_in_port) + PARM(mc_in_port) + PARM(io_in_port);
  info->n_out = PARM(out_port);
  info->n_cache_out = PARM(cache_out_port);
  info->n_mc_out = PARM(mc_out_port);
  info->n_io_out = PARM(io_out_port);
  info->n_total_out = PARM(out_port) + PARM(cache_out_port) +
                      PARM(mc_out_port) + PARM(io_out_port);
  ...
  /* separate buffer & shared switch implies buffer has tri-state output
   * driver*/
  ...
  if (info->in_buf) {
    ...
    SIM_array_init(&info->in_buf_info, 1, PARM(in_buf_rport), 1,
                   PARM(in_buf_set), PARM(flit_width), outdrv,
                   info->in_buffer_model);
  }
  ...
  if (PARM(cache_in_port)) {
    ...
    if (info->cache_in_buf) {
      ...
      SIM_array_init(&info->cache_in_buf_info, 1, PARM(cache_in_buf_rport), 1,
                     PARM(cache_in_buf_set), PARM(flit_width), outdrv, SRAM);
    }
  }
  ...
  if (PARM(mc_in_port)) {
    ...
    if (info->mc_in_buf) {
      ...
      SIM_array_init(&info->mc_in_buf_info, 1, PARM(mc_in_buf_rport), 1,
                     PARM(mc_in_buf_set), PARM(flit_width), outdrv, SRAM);
    }
  }
  ...
  if (PARM(io_in_port)) {
    ...
    if (info->io_in_buf) {
      ...
      SIM_array_init(&info->io_in_buf_info, 1, PARM(io_in_buf_rport), 1,
                     PARM(io_in_buf_set), PARM(flit_width), outdrv, SRAM);
    }
  }
  ...
  if (info->out_buf) {
    ...
    SIM_array_init(&info->out_buf_info, 1, 1, PARM(out_buf_wport),
                   PARM(out_buf_set), PARM(flit_width), 0,
                   info->out_buffer_model);
  }
  ...
  if (info->n_v_class * info->n_v_channel > 1) {
    if (info->sw_in_arb_model = PARM(sw_in_arb_model)) {
      if (PARM(sw_in_arb_model) == QUEUE_ARBITER) {
        SIM_array_init(&info->sw_in_arb_queue_info, 1, 1, 1,
                       info->n_v_class * info->n_v_channel,
                       SIM_logtwo(info->n_v_class * info->n_v_channel), 0,
                       REGISTER);
        if (info->cache_class > 1)
          SIM_array_init(&info->cache_in_arb_queue_info, 1, 1, 1,
                         info->cache_class, SIM_logtwo(info->cache_class), 0,
                         REGISTER);
        if (info->mc_class > 1)
          SIM_array_init(&info->mc_in_arb_queue_info, 1, 1, 1, info->mc_class,
                         SIM_logtwo(info->mc_class), 0, REGISTER);
        if (info->io_class > 1)
          SIM_array_init(&info->io_in_arb_queue_info, 1, 1, 1, info->io_class,
                         SIM_logtwo(info->io_class), 0, REGISTER);
  ...
  if (info->n_total_in > 2) {
    ...
    if (info->sw_out_arb_model) {
      if (info->sw_out_arb_model == QUEUE_ARBITER) {
        ...
        SIM_array_init(&info->sw_out_arb_queue_info, 1, 1, 1,
                       info->n_total_in - 1, line_width, 0, REGISTER);
  ...
  if (info->n_v_channel > 1 && info->n_in > 1) {
    if (info->vc_in_arb_model = PARM(vc_in_arb_model)) {
      if (PARM(vc_in_arb_model) == QUEUE_ARBITER) {
        SIM_array_init(&info->vc_in_arb_queue_info, 1, 1, 1, info->n_v_channel,
                       SIM_logtwo(info->n_v_channel), 0, REGISTER);
  ...
  if (info->n_in > 1 && info->n_v_channel > 1) {
    ...
    if (info->vc_out_arb_model) {
      if (info->vc_out_arb_model == QUEUE_ARBITER) {
        ...
        SIM_array_init(&info->vc_out_arb_queue_info, 1, 1, 1,
                       (info->n_total_in - 1) * info->n_v_channel, line_width,
                       0, REGISTER);
  ...
  if (info->vc_allocator_type == VC_SELECT && info->n_v_channel > 1 &&
      info->n_in > 1) {
    info->vc_select_buf_type = PARM(vc_select_buf_type);
    SIM_array_init(&info->vc_select_buf_info, 1, 1, 1, info->n_v_channel,
                   SIM_logtwo(info->n_v_channel), 0, info->vc_select_buf_type);
  }
  else {
  ...
  info->n_switch_in =
      info->n_in * info->in_n_switch + info->n_cache_in * info->cache_n_switch +
...
int SIM_buf_power_data_read(SIM_array_info_t *info, SIM_array_t *arr,
...
int SIM_buf_power_data_write(SIM_array_info_t *info, SIM_array_t *arr,

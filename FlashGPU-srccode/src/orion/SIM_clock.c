...
double SIM_total_clockEnergy(SIM_router_info_t *info,
  ...
  if (info->in_buf) {
    if (info->in_share_switch) {
      ...
      pipeline_regs += info->n_total_in * info->flit_width * info->n_v_channel *
                       info->n_v_class;
    }
  }
  ...
  /*for vc allocator and sw allocator, the clock power has been included in the
   * dynamic power part,
  ...
  if (info->H_tree_clock) {
    if ((PARM(TRANSISTOR_TYPE) == HVT) || (PARM(TRANSISTOR_TYPE) == NVT)) {
      H_tree_clockcap =
          (4 + 4 + 2 + 2) * (info->router_diagonal * 1e-6) * (Clockwire);
      H_tree_resistance =
          (4 + 4 + 2 + 2) * (info->router_diagonal * 1e-6) * (Reswire);

      int k;
      double h;
      int *ptr_k = &k;
      double *ptr_h = &h;
      getOptBuffering(ptr_k, ptr_h,
                      ((4 + 4 + 2 + 2) * (info->router_diagonal * 1e-6)));
      ...
      H_tree_clockcap =
          (8 + 4 + 4 + 4 + 4) * (info->router_diagonal * 1e-6) * (Clockwire);
      H_tree_resistance =
          (8 + 4 + 4 + 4 + 4) * (info->router_diagonal * 1e-6) * (Reswire);
      ...
      double *ptr_h = &h;
      getOptBuffering(ptr_k, ptr_h,
                      ((8 + 4 + 4 + 4 + 4) * (info->router_diagonal * 1e-6)));
  ...
  /* Here we divide ((pmosLeakage + nmosLeakage) / 2) by SCALE_S is because
   * pmosLeakage and nmosLeakage value is already for the specified technology,
   * doesn't need to use scaling factor. So we divide SCALE_S here first since
   * the value will be multiplied by SCALE_S later */
  router->I_clock_static = (((pmosLeakage + nmosLeakage) / 2) / SCALE_S +
                            (pipeline_regs * DFF_TAB[0] * Wdff));
...
#else
/*=================clock power is not supported for 110nm and
 * above=================*/
double SIM_total_clockEnergy(SIM_router_info_t *info,
                             SIM_router_power_t *router) {
  return 0;

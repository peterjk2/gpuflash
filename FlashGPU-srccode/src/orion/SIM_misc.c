...
/* Register renaming unit should be a separate module, which falls in array
 * structure category.  Dependency check logic is just the tag comparator of
 * that module. Following code is excerpted from Wattch as a reference in the
 * future */
...
static double SIM_iwin_sel_enc_cap(u_int level) {
  ...
  Ctotal =
      level * SIM_draincap(WSelPn, NCH, 1) + SIM_draincap(WSelPp, PCH, level);
...
/* WHS: maybe req should be of type LIB_Type_max_uint and use BIGONE instead of
 * 1 */
  ...
  if (encoding && encoding < BUS_MAX_ENC)
    switch (encoding) {
      ...
      case TRANS_ENC:
        return data_width;
      case BUSINV_ENC:
        return data_width + data_width / grp_width +
               (data_width % grp_width ? 1 : 0);
      default:
        printf("error\n"); /* some error handler */
    }
  else
...
/*
 * this function is provided to upper layers to compute the exact binary bus
 * representation only correct when grp_width divides data_width
 */
LIB_Type_max_uint SIM_bus_state(SIM_bus_t *bus, LIB_Type_max_uint old_data,
                                LIB_Type_max_uint old_state,
                                LIB_Type_max_uint new_data) {
  ...
  switch (bus->encoding) {
    ...
    case BUSINV_ENC:
      ...
      while (bus->data_width > done_width) {
        if (SIM_Hamming(old_state & mask_bus, new_data & mask_data, mask_bus) >
            bus->grp_width / 2)
          new_state += (~(new_data & mask_data) & mask_bus)
                       << done_width + done_width / bus->grp_width;
        else
          new_state += (new_data & mask_data)
                       << done_width + done_width / bus->grp_width;
...
static double SIM_resultbus_cap(void) {
  ...
  reg_height = PARM(RUU_size) *
               (RegCellHeight + WordlineSpacing * 3 * PARM(ruu_issue_width));
...
static double SIM_generic_bus_cap(u_int n_snd, u_int n_rcv, double length,
                                  double time) {
  double Ctotal = 0;
  ...
  if ((n_snd == 1) && (n_rcv == 1)) {
    ...
    Ctotal += SIM_gatecap(n_size + p_size, 0) + SIM_draincap(n_size, NCH, 1) +
              SIM_draincap(p_size, PCH, 1);
 ...
 */
int SIM_bus_init(SIM_bus_t *bus, int model, int encoding, u_int width,
  ...
  if ((bus->model = model) && model < BUS_MAX_MODEL) {
    ...
    switch (model) {
      ...
      case GENERIC_BUS:
        if ((bus->encoding = encoding) && encoding < BUS_MAX_ENC) {
          bus->e_switch = SIM_generic_bus_cap(n_snd, n_rcv, length, time) / 2 *
                          EnergyFactor;
          /* sanity check */
    ...
    // bus->static_af = 2 * (PARM(data_width) + npreg_width) *
    // PARM(ruu_issue_width) * PARM(AF);
    /* END: legacy */
...
int SIM_bus_record(SIM_bus_t *bus, LIB_Type_max_uint old_state,
...
/* this model is based on the gate-level design given by Randy H. Katz
 * "Contemporary Logic Design" Figure 6.24, node numbers (1-6) are assigned to
 * all gate outputs, left to right, top to bottom
 *
 * We should have pure cap functions and leave the decision of whether or not to
 * have coefficient 1/2 in init function.
 */
static double SIM_fpfp_node_cap(u_int fan_in, u_int fan_out) {
  ...
  Ctotal += fan_in * SIM_draincap(WdecNORn, NCH, 1) +
            SIM_draincap(WdecNORp, PCH, fan_in);
  ...
  if ((ff->model = model) && model < FF_MAX_MODEL) {
    switch (model) {
      case NEG_DFF:
        ...
        /* no 1/2 for e_keep and e_clock because clock signal switches twice in
         * one cycle */
        ...
        /* static power */
        ff->I_static = (WdecNORp * NOR2_TAB[0] +

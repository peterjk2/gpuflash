...
static int SIM_reg_port_state_init(SIM_power_array_info_t *info,
                                   SIM_reg_port_state_t *port, u_int n_port);

int FUNC(SIM_reg_power_init, SIM_power_array_info_t *info,
         SIM_power_array_t *arr, SIM_reg_port_state_t *read_port,
         SIM_reg_port_state_t *write_port) {
  /* ==================== set parameters ==================== */
  /* general parameters */
  info->share_rw = 0;
  ...
  if (read_port)
    SIM_reg_port_state_init(info, read_port, info->read_ports);
  if (write_port)
    SIM_reg_port_state_init(info, write_port, info->write_ports);

  return SIM_array_power_init(info, arr);
}

int FUNC(SIM_reg_power_report, SIM_power_array_info_t *info,
         SIM_power_array_t *arr) {
...
inline int FUNC(SIM_reg_power_dec, SIM_power_array_info_t *info,
                SIM_power_array_t *arr, u_int port, LIB_Type_max_uint row_addr,
                int rw) {
  if (rw) /* write */
    return SIM_power_array_dec(
        info, arr, (SIM_array_port_state_t *)&GLOB(reg_write_port)[port],
        row_addr, rw);
  else /* read */
    return SIM_power_array_dec(
        info, arr, (SIM_array_port_state_t *)&GLOB(reg_read_port)[port],
        row_addr, rw);
}

/* record read data activity */
inline int FUNC(SIM_reg_power_data_read, SIM_power_array_info_t *info,
                SIM_power_array_t *arr, P_DATA_T data) {
  return SIM_power_array_data_read(info, arr, data);
}

/* record write data bitline and memory cell activity */
inline int FUNC(SIM_reg_power_data_write, SIM_power_array_info_t *info,
                SIM_power_array_t *arr, u_int port, LIB_Type_max_uint old_data,
                LIB_Type_max_uint new_data) {
  return SIM_power_array_data_write(
      info, arr, NULL, sizeof(old_data),
      (u_char *)&GLOB(reg_write_port)[port].data_line, (u_char *)&old_data,
      (u_char *)&new_data);
}

/* record output driver activity */
inline int FUNC(SIM_reg_power_output, SIM_power_array_info_t *info,
                SIM_power_array_t *arr, P_DATA_T data) {

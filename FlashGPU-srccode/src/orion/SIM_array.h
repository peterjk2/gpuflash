...
typedef struct {
  ...
  /* tag various bits memory cells, different with tag_mem for fully-associative
   * cache */
 ...
 *
 *   entry           -- pointer to some entry structure if an entry is selected
 * for r/w, NULL otherwise entry_set       -- pointer to corresponding set
 * structure
 ...
 *
 * NOTE:
 *   (1) entry is interpreted by modules, if some module has no "entry
 * structure", then make sure this field is non-zero if some entry is selected
...
extern int SIM_array_dec(SIM_array_info_t *info, SIM_array_t *arr,
                         SIM_array_port_state_t *port,
                         LIB_Type_max_uint row_addr, int rw);
extern int SIM_array_data_read(SIM_array_info_t *info, SIM_array_t *arr,
                               LIB_Type_max_uint data);
extern int SIM_array_data_write(SIM_array_info_t *info, SIM_array_t *arr,
                                SIM_array_set_state_t *set, u_int n_item,
                                u_char *data_line, u_char *old_data,
                                u_char *new_data);
extern int SIM_array_tag_read(SIM_array_info_t *info, SIM_array_t *arr,
                              SIM_array_set_state_t *set);
extern int SIM_array_tag_update(SIM_array_info_t *info, SIM_array_t *arr,
                                SIM_array_port_state_t *port,
                                SIM_array_set_state_t *set);
extern int SIM_array_tag_compare(SIM_array_info_t *info, SIM_array_t *arr,
                                 SIM_array_port_state_t *port,
                                 LIB_Type_max_uint tag_input,
                                 LIB_Type_max_uint col_addr,
                                 SIM_array_set_state_t *set);
extern int SIM_array_output(SIM_array_info_t *info, SIM_array_t *arr,
                            u_int data_size, u_int length, void *data_out,
                            void *data_all);

/* state manupilation functions */
extern int SIM_array_port_state_init(SIM_array_info_t *info,
                                     SIM_array_port_state_t *port);
extern int SIM_array_set_state_init(SIM_array_info_t *info,
                                    SIM_array_set_state_t *set);

extern int SIM_array_init(SIM_array_info_t *info, int is_fifo,
                          u_int n_read_port, u_int n_write_port, u_int n_entry,
                          u_int line_width, int outdrv, int arr_buf_type);
...
extern double SIM_array_stat_energy(SIM_array_info_t *info, SIM_array_t *arr,
                                    double n_read, double n_write,
                                    int print_depth, char *path, int max_avg);

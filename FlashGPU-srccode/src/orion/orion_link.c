...
/*
 * link length in micro-meter - For consistency, user will enter the length in
 micro-meters, but in the code we multiply it by "1e-6" to convert it back to
 meters for internal calculations.
 */
...
int main(int argc, char **argv) {
  ...
  Pdynamic = 0.5 * load * LinkDynamicEnergyPerBitPerMeter(link_len, Vdd) *
             freq * link_len * (double)data_width;
  ...
  fprintf(
      stderr,
      "Link power and area are only supported for 90nm, 65nm, 45nm and 32nm\n");
#endif

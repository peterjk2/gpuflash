...
static double SIM_omflip_in_cap(double length) {
  ...
  Ctotal +=
      2 * SIM_draincap(WomfPASSn, NCH, 1) + SIM_draincap(WomfPASSp, PCH, 1);
...
static double SIM_omflip_int_cap(void) {
  ...
  Ctotal +=
      SIM_draincap(WomfPASSn, NCH, 1) + 2 * SIM_draincap(WomfPASSp, PCH, 1);
...
inline static LIB_Type_max_uint SIM_omflip_omega(LIB_Type_max_uint in,
                                                 u_int width,
                                                 LIB_Type_max_uint mask,
                                                 int pass,
                                                 LIB_Type_max_uint ctr) {
...
inline static LIB_Type_max_uint SIM_omflip_flip(LIB_Type_max_uint in,
                                                u_int width, int pass,
                                                LIB_Type_max_uint ctr) {
...
static double SIM_grp_zin_cap(u_int stg) {
  ...
  if (stg == 1)
    Ctotal +=
        SIM_draincap(Wgrp2NANDn, NCH, 2) + 2 * SIM_draincap(Wgrp2NANDp, PCH, 1);
  else {
    n_track = (1 << stg - 2) + 1;
    /* wire cap */
    ...
    Ctotal += n_track * (SIM_draincap(WgrpPASSn, NCH, 1) +
                         SIM_draincap(WgrpPASSp, PCH, 1));
  }
  ...
  Ctotal +=
      (0.75 * n_track * 2 * WordlineSpacing + (n_track - 1) * BitlineSpacing) *
      CCmetal;

  /* part 3: this stage drain cap */
  /* WHS: use average track: (3/4 * n) */
  Ctotal += 0.75 * n_track *
            (SIM_draincap(WgrpPASSn, NCH, 1) + SIM_draincap(WgrpPASSp, PCH, 1));
...
static double SIM_grp_lin_cap(u_int stg) {
  ...
  if (stg == 1) {
    /* input inverter */
    Ctotal += SIM_draincap(WgrpL1NOTn, NCH, 1) +
              SIM_draincap(WgrpL1NOTp, PCH, 1) +
  ...
  if (stg > 1)
    Ctotal +=
        SIM_draincap(WgrpL1NOTn, NCH, 1) + SIM_draincap(WgrpL1NOTp, PCH, 1) +
        SIM_gatecap(WgrpL1NOTn + WgrpL1NOTp, 0) +
  ...
  if (stg > 4)
    Ctotal += SIM_draincap(WgrpL4NOTn, NCH, 1) +
              SIM_draincap(WgrpL4NOTp, PCH, 1) +
...
static double SIM_grp_rin_cap(u_int stg) {
  ...
  n_track = (1 << stg - 1) + 1;
  Ctotal +=
      (n_track * 2 * WordlineSpacing + (n_track - 1) * 2 * BitlineSpacing) *
      CCmetal;
...
static double SIM_grp_out_cap(void) {
  return (2 * SIM_draincap(Wgrp2NORn, NCH, 1) +
          SIM_draincap(Wgrp2NORp, PCH, 2));
}

static int SIM_grp_reset(SIM_grp_t *grp) {
  grp->in = grp->out = grp->zoin = grp->woin = 0;
  grp->zin[0] = grp->zin[1] = grp->zin[2] = grp->zin[3] = grp->zin[4] =
      grp->zin[5] = 0;
  grp->win[0] = grp->win[1] = grp->win[2] = grp->win[3] = grp->win[4] =
      grp->win[5] = 0;
...
int SIM_permu_init(SIM_permu_t *permu, int model, u_int data_width) {
  ...
  assert(data_width && data_width <= 8 * sizeof(LIB_Type_max_uint) &&
         !(data_width % 2));

  switch (permu->model = model) {
    case OMFLIP_PERMU:
      permu->u.omflip.data_width = data_width;
      permu->u.omflip.n_chg_in = permu->u.omflip.n_chg_out =
          permu->u.omflip.n_chg_stg = 0;
      permu->u.omflip.n_chg_int = permu->u.omflip.n_chg_pass =
          permu->u.omflip.n_chg_ctr = 0;
      /* initial state */
      ...
      permu->u.omflip.e_chg_in =
          SIM_omflip_in_cap(unit_dist +
                            (0.5 + 0.25 * data_width) * unit_height) /
          2 * EnergyFactor;
      permu->u.omflip.e_chg_out = SIM_omflip_out_cap() / 2 * EnergyFactor;
      /* WHS: use average line length */
      permu->u.omflip.e_chg_stg =
          SIM_omflip_stg_cap(unit_dist +
                             (0.5 + 1.25 * data_width) * WordlineSpacing) /
          2 * EnergyFactor;
      permu->u.omflip.e_chg_int = SIM_omflip_int_cap() / 2 * EnergyFactor;
      permu->u.omflip.e_chg_pass =
          SIM_omflip_ctr_cap(data_width, data_width * unit_height) / 2 *
          EnergyFactor;
      /* WHS: use average control line length */
      permu->u.omflip.e_chg_ctr =
          SIM_omflip_ctr_cap(2, data_width * unit_height / 2) / 2 *
          EnergyFactor;
      break;

    case GRP_PERMU:
      permu->u.grp.data_width = data_width;
      permu->u.grp.n_chg_in = permu->u.grp.n_chg_oin = permu->u.grp.n_chg_out =
          0;
      for (stg = 0; stg < 6; stg++) {
      ...
      for (stg = 1; stg <= permu->u.grp.n_stg; stg++) {
        permu->u.grp.e_chg_zin[stg - 1] =
            SIM_grp_zin_cap(stg) / 2 * EnergyFactor;
        /* 2 one-hot signals always switch simultaneously, and shared by z/w */
        permu->u.grp.e_chg_lin[stg - 1] =
            SIM_grp_lin_cap(stg) * 2 * EnergyFactor;
        permu->u.grp.e_chg_rin[stg - 1] =
            SIM_grp_rin_cap(stg) * 2 * EnergyFactor;
      }
...
inline static int SIM_omflip_record(SIM_omflip_t *omflip, LIB_Type_max_uint in,
                                    u_int mode, LIB_Type_max_uint op,
                                    int reset) {
  u_int pass = 0;
  LIB_Type_max_uint stg, ctr;

  if (reset)
    SIM_omflip_reset(omflip);
  ...
  if (is_omega(mode & FIRST_STG)) {
    ...
    omflip->inn[0] = in;
    stg =
        SIM_omflip_omega(in, omflip->data_width / 2, omflip->ctr_mask, 0, ctr);
  }
  else {
    pass = pass | FIRST_STG;
  ...
  if (is_omega(mode & SECOND_STG)) {
    ...
    omflip->inn[3] = stg;
    stg =
        SIM_omflip_omega(stg, omflip->data_width / 2, omflip->ctr_mask, 0, ctr);
  }
  else {
...
inline static LIB_Type_max_uint SIM_omflip_record_test(SIM_omflip_t *omflip,
                                                       LIB_Type_max_uint in,
                                                       u_int mode,
                                                       LIB_Type_max_uint op) {
  u_int pass = 0;
  LIB_Type_max_uint stg, ctr;
...
/* stg <= 6 */
inline static LIB_Type_max_uint SIM_grp_stg(LIB_Type_max_uint zin, u_int *lin,
                                            u_int width, u_int stg, int zw) {
...
inline static int SIM_grp_record(SIM_grp_t *grp, LIB_Type_max_uint in,
                                 LIB_Type_max_uint op, int reset) {
...
inline static LIB_Type_max_uint SIM_grp_record_test(SIM_grp_t *grp,
                                                    LIB_Type_max_uint in,
                                                    LIB_Type_max_uint op) {
  u_int i, stg, base = 0, lin[32], rin[32];
  LIB_Type_max_uint zin, win;
...
int SIM_permu_record(SIM_permu_t *permu, LIB_Type_max_uint in, u_int mode,
                     LIB_Type_max_uint op, int reset) {
  switch (permu->model) {
    case OMFLIP_PERMU:
...
LIB_Type_max_uint SIM_permu_record_test(SIM_permu_t *permu,
                                        LIB_Type_max_uint in, u_int mode,
                                        LIB_Type_max_uint op) {
  switch (permu->model) {
    case OMFLIP_PERMU:
...
double SIM_permu_max_energy(SIM_permu_t *permu) {
  ...
  switch (permu->model) {
    case OMFLIP_PERMU:
      ...
      // SIM_omflip_record_test(&permu->u.omflip, permu->u.omflip.data_mask,
      // 0x03, permu->u.omflip.data_mask); return SIM_permu_report(permu);
    ...
    case GRP_PERMU:
      ...
      // SIM_grp_record_test(&permu->u.grp, permu->u.grp.mask,
      // permu->u.grp.mask); return SIM_permu_report(permu);

...
double computeResistance(double Length) {
  ...
  if (widthSpacingConfig == SWIDTH_SSPACE) {
    ...
    r = ((rho * Length) /
         ((WireMinWidth - 2 * WireBarrierThickness) *
    ...
    // rho is in "ohm.m"
    rho =
        2.202e-8 + (1.030e-15 / (2 * WireMinWidth - 2 * WireBarrierThickness));
    r = ((rho * Length) /
         ((2 * WireMinWidth - 2 * WireBarrierThickness) *
          (WireMetalThickness - WireBarrierThickness)));  // r is in "ohm"
  }
...
double computeGroundCapacitance(double Length) {
  ...
  if (widthSpacingConfig == SWIDTH_SSPACE) {
    ...
    double B = 2.04 * pow((WireMinSpacing /
                           (WireMinSpacing + 0.54 * WireDielectricThickness)),
                          1.77);
    double C = pow((WireMetalThickness /
                    (WireMetalThickness + 4.53 * WireDielectricThickness)),
                   0.07);
    ...
    double B =
        2.04 *
        pow((minSpacingNew / (minSpacingNew + 0.54 * WireDielectricThickness)),
            1.77);
    double C = pow((WireMetalThickness /
                    (WireMetalThickness + 4.53 * WireDielectricThickness)),
                   0.07);
...
double computeCouplingCapacitance(double Length) {
  ...
  if (widthSpacingConfig == SWIDTH_SSPACE) {
    double A = 1.14 * (WireMetalThickness / WireMinSpacing) *
               exp(-4 * WireMinSpacing /
                   (WireMinSpacing + 8.01 * WireDielectricThickness));
    double B =
        2.37 *
        pow((WireMinWidth / (WireMinWidth + 0.31 * WireMinSpacing)), 0.28);
    double C = pow((WireDielectricThickness /
                    (WireDielectricThickness + 8.96 * WireMinSpacing)),
                   0.76) *
               exp(-2 * WireMinSpacing /
                   (WireMinSpacing + 6 * WireDielectricThickness));
    ...
    double A = 1.14 * (WireMetalThickness / minSpacingNew) *
               exp(-4 * minSpacingNew /
                   (minSpacingNew + 8.01 * WireDielectricThickness));
    double B =
        2.37 *
        pow((WireMinWidth / (WireMinWidth + 0.31 * minSpacingNew)), 0.28);
    double C =
        pow((WireDielectricThickness /
             (WireDielectricThickness + 8.96 * minSpacingNew)),
            0.76) *
    ...
    double B =
        2.37 *
        pow((2 * minWidthNew / (2 * minWidthNew + 0.31 * WireMinSpacing)),
            0.28);
    double C = pow((WireDielectricThickness /
                    (WireDielectricThickness + 8.96 * WireMinSpacing)),
                   0.76) *
    ...
    double A = 1.14 * (WireMetalThickness / minSpacingNew) *
               exp(-4 * minSpacingNew /
                   (minSpacingNew + 8.01 * WireDielectricThickness));
    double B =
        2.37 * pow((minWidthNew / (minWidthNew + 0.31 * minSpacingNew)), 0.28);
    double C =
        pow((WireDielectricThickness /
             (WireDielectricThickness + 8.96 * minSpacingNew)),
            0.76) *
  ...
  if (PARM(buffering_scheme) == MIN_DELAY) {
    if (PARM(shielding)) {
      ...
      *k = (int)sqrt(((0.4 * r * c_g) + (0.57 * r * c_c)) /
                     (0.7 * BufferDriveResistance *
                      BufferInputCapacitance));  // k is the number of buffers
                                                 // to be inserted
      *h = sqrt(
          ((0.7 * BufferDriveResistance * c_g) +
           (1.4 * 1.5 * BufferDriveResistance * c_c)) /
          (0.7 * r *
           BufferInputCapacitance));  // the size of the buffers to be inserted
    }
    else {
      double r = computeResistance(Length);
      double c_g = 2 * computeGroundCapacitance(Length);
      ...
      *h = sqrt(((0.7 * BufferDriveResistance * c_g) +
                 (1.4 * 2.2 * BufferDriveResistance * c_c)) /
                (0.7 * r * BufferInputCapacitance));
    }
  }
...
double LinkArea(double Length, unsigned NumBits) {
  ...
  routingArea = (NumBits * (WireMinWidth * 1e6 + WireMinSpacing * 1e6) +
                 WireMinSpacing * 1e6) *
                Length * 1e6;

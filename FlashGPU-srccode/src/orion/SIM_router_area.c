...
int SIM_router_area_init(SIM_router_info_t *info,
  ...
  if (info->in_buf) {
    switch (info->in_buffer_model) {
      case SRAM:
        bitline_len =
            info->in_buf_info.n_set * (RegCellHeight + 2 * WordlineSpacing);
        wordline_len = info->flit_width *
                       (RegCellWidth + 2 *
                                           (info->in_buf_info.read_ports +
                                            info->in_buf_info.write_ports) *
                                           BitlineSpacing);

        /* input buffer area */
        router_area->buffer = info->n_in * info->n_v_class *
                              (bitline_len * wordline_len) *
                              (info->in_share_buf ? 1 : info->n_v_channel);
        break;

      case REGISTER:
        router_area->buffer = AreaDFF * info->n_in * info->n_v_class *
                              info->flit_width * info->in_buf_info.n_set *
                              (info->in_share_buf ? 1 : info->n_v_channel);
        break;
  ...
  if (info->out_buf) {
    switch (info->out_buffer_model) {
      case SRAM:
        bitline_len =
            info->out_buf_info.n_set * (RegCellHeight + 2 * WordlineSpacing);
        wordline_len = info->flit_width *
                       (RegCellWidth + 2 *
                                           (info->out_buf_info.read_ports +
                                            info->out_buf_info.write_ports) *
                                           BitlineSpacing);

        /* output buffer area */
        router_area->buffer += info->n_out * info->n_v_class *
                               (bitline_len * wordline_len) *
                               (info->out_share_buf ? 1 : info->n_v_channel);
        break;

      case REGISTER:
        router_area->buffer += AreaDFF * info->n_out * info->n_v_class *
                               info->flit_width * info->out_buf_info.n_set *
                               (info->out_share_buf ? 1 : info->n_v_channel);
        break;
    ...
    switch (info->crossbar_model) {
      ...
      case MULTREE_CROSSBAR:
        if (info->degree == 2) {
          ...
          router_area->crossbar = info->n_switch_in * info->flit_width *
                                  boxArea * info->n_switch_out;
        }
  ...
  if (info->exp_xb_model) {  // only support for MATRIX_CROSSBAR type
    xb_in_len =
        (2 * info->n_switch_in - 1) * info->flit_width * CrsbarCellWidth;
    xb_out_len =
        (2 * info->n_switch_out - 1) * info->flit_width * CrsbarCellHeight;
    router_area->crossbar = xb_in_len * xb_out_len;
  }
    ...
    switch (info->sw_in_arb_model) {
      case MATRIX_ARBITER:  // assumes 30% spacing for each arbiter
        router_area->sw_allocator +=
            ((AreaNOR * 2 * (req_width - 1) * req_width) +
             (AreaINV * req_width) +
             (AreaDFF * (req_width * (req_width - 1) / 2))) *
            1.3 * info->in_n_switch * info->n_in;
        break;

      case RR_ARBITER:  // assumes 30% spacing for each arbiter
        router_area->sw_allocator +=
            ((6 * req_width * AreaNOR) + (2 * req_width * AreaINV) +
             (req_width * AreaDFF)) *
            1.3 * info->in_n_switch * info->n_in;
        break;

      case QUEUE_ARBITER:
        router_area->sw_allocator += AreaDFF *
  ...
  if (info->sw_out_arb_model) {
    ...
    switch (info->sw_out_arb_model) {
      ...
      case QUEUE_ARBITER:
        router_area->sw_allocator +=
            AreaDFF * info->sw_out_arb_queue_info.data_width *
  ...
  if (info->vc_allocator_type == ONE_STAGE_ARB && info->n_v_channel > 1 &&
    ...
    if (info->vc_out_arb_model) {
      ...
      switch (info->vc_out_arb_model) {
        case MATRIX_ARBITER:  // assumes 30% spacing for each arbiter
          router_area->vc_allocator =
              ((AreaNOR * 2 * (req_width - 1) * req_width) +
               (AreaINV * req_width) +
               (AreaDFF * (req_width * (req_width - 1) / 2))) *
              1.3 * info->n_out * info->n_v_channel * info->n_v_class;
          break;
        ...
        case QUEUE_ARBITER:
          router_area->vc_allocator =
              AreaDFF * info->vc_out_arb_queue_info.data_width *
              info->vc_out_arb_queue_info.n_set * info->n_out *
    ...
    if (info->vc_in_arb_model && info->vc_out_arb_model) {
      ...
      switch (info->vc_in_arb_model) {
        case MATRIX_ARBITER:  // assumes 30% spacing for each arbiter
          router_area->vc_allocator =
              ((AreaNOR * 2 * (req_width - 1) * req_width) +
               (AreaINV * req_width) +
               (AreaDFF * (req_width * (req_width - 1) / 2))) *
              1.3 * info->n_in * info->n_v_channel * info->n_v_class;
          break;
        ...
        case QUEUE_ARBITER:
          router_area->vc_allocator =
              AreaDFF * info->vc_in_arb_queue_info.data_width *
              info->vc_in_arb_queue_info.n_set * info->n_in *
              info->n_v_channel * info->n_v_class;
    ...
    switch (info->vc_select_buf_type) {
      case SRAM:
        ...
        wordline_len =
            ...
            (RegCellWidth + 2 *
                                (info->vc_select_buf_info.read_ports +
                                 info->vc_select_buf_info.write_ports) *
                                BitlineSpacing);
        router_area->vc_allocator =
            info->n_out * info->n_v_class * (bitline_len * wordline_len);

        break;

      case REGISTER:
        router_area->vc_allocator = AreaDFF * info->n_out * info->n_v_class *
                                    info->vc_select_buf_info.data_width *
...
double SIM_router_area(SIM_router_area_t *router_area) {
  double Atotal;
  Atotal = router_area->buffer + router_area->crossbar +
           router_area->vc_allocator + router_area->sw_allocator;

#if (PARM(TECH_POINT) <= 90)
  fprintf(stdout,
          "Abuffer:%g\t ACrossbar:%g\t AVCAllocator:%g\t ASWAllocator:%g\t "
          "Atotal:%g\n",
          router_area->buffer, router_area->crossbar, router_area->vc_allocator,
          router_area->sw_allocator, Atotal);

#else
  fprintf(stderr,
          "Router area is only supported for 90nm, 65nm, 45nm and 32nm\n");
#endif
  return Atotal;
}

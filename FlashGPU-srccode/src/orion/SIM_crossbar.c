...
static double SIM_crossbar_in_cap(double wire_cap, u_int n_out, double n_seg,
                                  int connect_type, int trans_type,
                                  double *Nsize) {
  ...
  if (n_seg > 1) {
    ...
    Ctotal += (n_seg + 2) * (n_seg - 1) / (n_seg * 2) *
              (SIM_gatecap(Woutdrvnandn + Woutdrvnandp, 0) +
               SIM_gatecap(Woutdrvnorn + Woutdrvnorp, 0));
...
static double SIM_crossbar_out_cap(double length, u_int n_in, double n_seg,
                                   int connect_type, int trans_type,
                                   double *Nsize) {
  double Ctotal = 0, Ctrans, psize, nsize;
  ...
  Ctotal += SIM_draincap(Woutdrivern, NCH, 1) +
...
static double SIM_crossbar_io_cap(double length) {
  ...
  Ctotal += 2 * (SIM_draincap(Woutdrivern, NCH, 1) +
                 SIM_draincap(Woutdriverp, PCH, 1));
...
static double SIM_crossbar_int_cap(u_int degree, int connect_type,
                                   int trans_type) {
  ...
  if (connect_type == TRANS_GATE) {
    ...
    Ctotal += degree * (SIM_draincap(Woutdrivern, NCH, 1) +
                        SIM_draincap(Woutdriverp, PCH, 1));
...
static double SIM_crossbar_ctr_cap(double length, u_int data_width,
                                   int prev_ctr, int next_ctr, u_int degree,
                                   int connect_type, int trans_type) {
  ...
  if (prev_ctr)
    ...
    Ctotal += degree * SIM_draincap(WdecNORn, NCH, 1) +
              SIM_draincap(WdecNORp, PCH, degree);
...
int SIM_crossbar_init(SIM_crossbar_t *crsbar, int model, u_int n_in,
                      u_int n_out, u_int in_seg, u_int out_seg,
                      u_int data_width, u_int degree, int connect_type,
                      int trans_type, double in_len, double out_len,
                      double *req_len) {
  ...
  if ((crsbar->model = model) && model < CROSSBAR_MAX_MODEL) {
    ...
    crsbar->n_chg_in = crsbar->n_chg_int = crsbar->n_chg_out =
        crsbar->n_chg_ctr = 0;
    ...
    switch (model) {
      case MATRIX_CROSSBAR:
        ...
        crsbar->e_chg_out =
            SIM_crossbar_out_cap(out_length, n_in, out_seg, connect_type,
                                 trans_type, &Nsize) *
            EnergyFactor;
        crsbar->e_chg_in =
            SIM_crossbar_in_cap(in_wire_cap, n_out, in_seg, connect_type,
                                trans_type, &Nsize) *
            EnergyFactor;
        /* FIXME: wire length estimation, really reset? */
        /* control signal should reset after transmission is done, so no 1/2 */
        crsbar->e_chg_ctr = SIM_crossbar_ctr_cap(ctr_length, data_width, 0, 0,
                                                 0, connect_type, trans_type) *
                            EnergyFactor;
        crsbar->e_chg_int = 0;
        ...
        /* tri-state buffers */
        I_static +=
            ((Woutdrvnandp * (NAND2_TAB[0] + NAND2_TAB[1] + NAND2_TAB[2]) +
              Woutdrvnandn * NAND2_TAB[3]) /
                 4 +
             (Woutdrvnorp * NOR2_TAB[0] +
              Woutdrvnorn * (NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3])) /
                 4 +
             Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) *
            n_in * n_out * data_width;
        /* input driver */
        I_static += (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in *
                    data_width;
        /* output driver */
        I_static += (Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) *
                    n_out * data_width;
        /* control signal inverter */
        I_static +=
            (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in * n_out;
      ...
      case MULTREE_CROSSBAR:
        ...
        crsbar->e_chg_out =
            SIM_crossbar_out_cap(0, degree, 0, connect_type, trans_type, NULL) *
            EnergyFactor;
        crsbar->e_chg_in = SIM_crossbar_in_cap(in_wire_cap, n_out, 0,
                                               connect_type, trans_type, NULL) *
                           EnergyFactor;
        crsbar->e_chg_int =
            SIM_crossbar_int_cap(degree, connect_type, trans_type) *
            EnergyFactor;
        ...
        if (crsbar->depth == 1)
          ...
          crsbar->e_chg_ctr =
              SIM_crossbar_ctr_cap(ctr_length, data_width, 0, 0, degree,
                                   connect_type, trans_type) *
              EnergyFactor;
        else {
          /* first level and last level control signals */
          crsbar->e_chg_ctr =
              SIM_crossbar_ctr_cap(ctr_length, data_width, 0, 1, degree,
                                   connect_type, trans_type) *
                  EnergyFactor +
              SIM_crossbar_ctr_cap(0, data_width, 1, 0, degree, connect_type,
                                   trans_type) *
                  EnergyFactor;
          ...
          if (crsbar->depth > 2)
            crsbar->e_chg_ctr +=
                (crsbar->depth - 2) *
                SIM_crossbar_ctr_cap(0, data_width, 1, 1, degree, connect_type,
                                     trans_type) *
                EnergyFactor;
        }
        ...
        I_static += (Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) *
                    n_out * data_width;
        /* mux */
        I_static += (WdecNORp * NOR2_TAB[0] +
                     WdecNORn * (NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3])) /
                    4 * (2 * n_in - 1) * n_out * data_width;
        /* control signal inverter */
        I_static +=
            (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in * n_out;
        crsbar->I_static = I_static;
...
int SIM_crossbar_record(SIM_crossbar_t *xb, int io, LIB_Type_max_uint new_data,
                        LIB_Type_max_uint old_data, u_int new_port,
                        u_int old_port) {
  switch (xb->model) {
    case MATRIX_CROSSBAR:
      if (io) /* input port */
...
double SIM_crossbar_report(SIM_crossbar_t *crsbar) {
  return (crsbar->n_chg_in * crsbar->e_chg_in +
          crsbar->n_chg_out * crsbar->e_chg_out +
          crsbar->n_chg_int * crsbar->e_chg_int +
          crsbar->n_chg_ctr * crsbar->e_chg_ctr);
}

double SIM_crossbar_stat_energy(SIM_crossbar_t *crsbar, int print_depth,
                                char *path, int max_avg, double n_data) {
  ...
  switch (crsbar->model) {
    ...
    case MULTREE_CROSSBAR:
      ...
      Eatomic =
          crsbar->e_chg_in * crsbar->data_width * (max_avg ? 1 : 0.5) * n_data;
      SIM_print_stat_energy(SIM_strcat(path, "input"), Eatomic, next_depth);
      SIM_res_path(path, path_len);
      Eavg += Eatomic;

      Eatomic =
          crsbar->e_chg_out * crsbar->data_width * (max_avg ? 1 : 0.5) * n_data;
      ...
      if (crsbar->model == MULTREE_CROSSBAR && crsbar->depth > 1) {
        Eatomic = crsbar->e_chg_int * crsbar->data_width * (crsbar->depth - 1) *
                  (max_avg ? 1 : 0.5) * n_data;

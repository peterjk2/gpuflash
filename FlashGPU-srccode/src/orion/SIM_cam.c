...
double SIM_cam_comp_tagline_cap(u_int rows, double taglinelength) {
  ...
  /* part 2: input driver */
  Ctotal += SIM_draincap(Wcompdrivern, NCH, 1) +
            SIM_draincap(Wcompdriverp, PCH, 1) +
...
double SIM_cam_comp_mismatch_cap(u_int n_bits, u_int n_pre,
                                 double matchline_len) {
  double Ctotal;
  ...
  Ctotal += n_bits * (SIM_draincap(Wcomparen1, NCH, 1) +
                      SIM_draincap(Wcomparen1, NCH, 2));
...
double SIM_cam_comp_miss_cap(u_int assoc) {
  ...
  return (assoc * SIM_draincap(Wmatchnorn, NCH, 1) +
          SIM_draincap(Wmatchnorp, PCH, assoc));
}
...
double SIM_cam_tag_mem_cap(u_int read_ports, u_int write_ports, int share_rw,
                           u_int end, int only_write) {
  double Ctotal;

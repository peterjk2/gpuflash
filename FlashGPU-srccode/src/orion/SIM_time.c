...
double SIM_gatecap(double width, double wirelength) {
  ...
  gateCap = ((width - 2 * Pdelta_w) * (l * LSCALE - 2 * Pdelta_l) * PCg) +
            2.0 * overlapCap;

  return gateCap;
#endif
  return (width * Leff * PARM(Cgate) * SCALE_T +
          wirelength * Cpolywire * Leff * SCALE_T);
...
double SIM_draincap(double width, int nchannel, int stack) {
  ...
  if (nchannel == 0) {
    ...
    area_peri = ((diffArea * PCja) + (diffPeri * PCjsw));
    ...
    area_peri = ((diffArea * NCja * LSCALE) + (diffPeri * NCjsw * LSCALE));
 ...
 */
double SIM_horowitz(double inputramptime, double tf, double vs1, double vs2,
                    int rise) {
  double a, b, td;
...
double SIM_decoder_delay(int C, int B, int A, int Ndwl, int Ndbl, int Nspd,
                         int Ntwl, int Ntbl, int Ntspd, double *Tdecdrive,
                         double *Tdecoder1, double *Tdecoder2,
                         double *outrisetime) {
  ...
  nextinputtime =
      SIM_horowitz(z, tf, PARM(VTHINV100x60), PARM(VTHINV100x60), FALL) /
      (PARM(VTHINV100x60));

  Ceq = SIM_draincap(Wdecdrivep, PCH, 1) + SIM_draincap(Wdecdriven, NCH, 1) +
        SIM_gatecap(Wdecdrivep + Wdecdriven, 0.0);
  tf = Ceq * SIM_transreson(Wdecdriven, NCH, 1);
  nextinputtime = SIM_horowitz(nextinputtime, tf, PARM(VTHINV100x60),
                               PARM(VTHINV100x60), RISE) /
  ...
  *Tdecdrive = SIM_horowitz(nextinputtime, tf, PARM(VTHINV100x60),
                            PARM(VTHNAND60x90), FALL);
...
double SIM_decoder_tag_delay(int C, int B, int A, int Ndwl, int Ndbl, int Nspd,
                             int Ntwl, int Ntbl, int Ntspd, double *Tdecdrive,
                             double *Tdecoder1, double *Tdecoder2,
                             double *outrisetime) {
  double Ceq, Req, Rwire, rows, tf, nextinputtime, vth = 0, tstep, m, a, b, c;
  ...
  tf = Ceq * SIM_transreson(Wdecdriven, NCH, 1);
  nextinputtime =
      SIM_horowitz(0.0, tf, PARM(VTHINV100x60), PARM(VTHINV100x60), FALL) /
      (PARM(VTHINV100x60));
...
double SIM_wordline_delay(int B, int A, int Ndwl, int Nspd, double inrisetime,
                          double *outrisetime) {
  ...
  Cline = (SIM_gatecappass(Wmemcella, 0.0) + SIM_gatecappass(Wmemcella, 0.0) +
           Cwordmetal) *
          cols;
  ...
  Twordchargedel =
      SIM_horowitz(nextinputtime, tf, PARM(VSINV), PARM(VSINV), FALL);
...
double SIM_wordline_tag_delay(int C, int A, int Ntspd, int Ntwl,
  ...
  tagbits =
      PARM(ADDRESS_BITS) + 2 - (int)logtwo((double)C) + (int)logtwo((double)A);
...
double SIM_bitline_delay(int C, int A, int B, int Ndwl, int Ndbl, int Nspd,
                         double inrisetime, double *outrisetime) {
  ...
  if (Ndbl * Nspd == 1) {
    ...
    Cline = rows * (Cbitrow + Cbitmetal) +
            2 * SIM_draincap(Wbitpreequ, PCH, 1) +
            SIM_draincap(Wbitmuxn, NCH, 1);
    Ccolmux = Nspd * Ndbl * (SIM_draincap(Wbitmuxn, NCH, 1)) +
              2 * SIM_gatecap(WsenseQ1to4, 10.0);
    Rlineb = Rbitmetal * rows / 2.0;
    r1 = Rlineb + SIM_transreson(Wbitmuxn, NCH, 1);
  }
...
double SIM_bitline_tag_delay(int C, int A, int B, int Ntwl, int Ntbl, int Ntspd,
                             double inrisetime, double *outrisetime) {
  double Tbit, Cline, Ccolmux, Rlineb, r1, r2, c1, c2, a, b, c;
  double m, tstep;
  double Cbitrow; /* bitline capacitance due to access transistor */
  ...
  if (Ntbl * Ntspd == 1) {
    ...
    Cline = rows * (Cbitrow + Cbitmetal) +
            2 * SIM_draincap(Wbitpreequ, PCH, 1) +
            SIM_draincap(Wbitmuxn, NCH, 1);
    Ccolmux = Ntspd * Ntbl * (SIM_draincap(Wbitmuxn, NCH, 1)) +
              2 * SIM_gatecap(WsenseQ1to4, 10.0);
    Rlineb = Rbitmetal * rows / 2.0;
    r1 = Rlineb + SIM_transreson(Wbitmuxn, NCH, 1);
  }
...
double SIM_compare_time(int C, int A, int Ntbl, int Ntspd, double inputtime,
                        double *outputtime) {
  ...
  tf = Req * Ceq;
  st1del =
      SIM_horowitz(inputtime, tf, PARM(VTHCOMPINV), PARM(VTHCOMPINV), FALL);
  ...
  tf = Req * Ceq;
  st2del =
      SIM_horowitz(inputtime, tf, PARM(VTHCOMPINV), PARM(VTHCOMPINV), RISE);
  ...
  tf = Req * Ceq;
  st3del =
      SIM_horowitz(nextinputtime, tf, PARM(VTHCOMPINV), PARM(VTHEVALINV), FALL);
  nextinputtime = st1del / (PARM(VTHEVALINV));

  /* Final Inverter (virtual ground driver) discharging compare part */

  tagbits =
      PARM(ADDRESS_BITS) - (int)logtwo((double)C) + (int)logtwo((double)A);
  ...
  c1 = (tagbits) *
           (SIM_draincap(Wcompn, NCH, 1) + SIM_draincap(Wcompn, NCH, 2)) +
       SIM_draincap(Wcompp, PCH, 1) +
       SIM_gatecap(Wmuxdrv12n + Wmuxdrv12p, 20.0) + cols * Cwordmetal;
  ...
  if ((tstep) <= (0.5 * (Vdd - Vt) / m)) {
    ...
    c = -2 * (tstep) * (Vdd - Vt) + 1 / m * ((Vdd * PARM(VTHEVALINV)) - Vt) *
                                        ((Vdd * PARM(VTHEVALINV)) - Vt);
    Tcomparatorni = (-b + sqrt(b * b - 4 * a * c)) / (2 * a);
  }
  else {
    Tcomparatorni =
        (tstep) + (Vdd + Vt) / (2 * m) - (Vdd * PARM(VTHEVALINV)) / m;
  }
...
double SIM_mux_driver_delay(int C, int B, int A, int Ndbl, int Nspd, int Ndwl,
                            int Ntbl, int Ntspd, double inputtime,
                            double *outputtime) {
  ...
  Ceq = SIM_gatecap(Wmuxdrv3n + Wmuxdrv3p, 15.0) +
        2 * SIM_draincap(WmuxdrvNORn, NCH, 1) +
        SIM_draincap(WmuxdrvNORp, PCH, 2);
  Req = SIM_transreson(WmuxdrvNORn, NCH, 1);
  tf = Ceq * Req;
  Tst2 =
      SIM_horowitz(nextinputtime, tf, PARM(VTHMUXDRV2), PARM(VTHMUXDRV3), RISE);
  nextinputtime = Tst2 / (1 - PARM(VTHMUXDRV3));

  /* third driver stage - invert "select" to produce "select bar" */

  Ceq = PARM(BITOUT) *
            SIM_gatecap(Woutdrvseln + Woutdrvselp + Woutdrvnorn + Woutdrvnorp,
                        20.0) +
        SIM_draincap(Wmuxdrv3p, PCH, 1) + SIM_draincap(Wmuxdrv3n, NCH, 1) +
        Cwordmetal * 8 * B * A * Nspd * Ndbl / 2.0;
  Req = (Rwordmetal * 8 * B * A * Nspd * Ndbl / 2) / 2 +
        SIM_transreson(Wmuxdrv3p, PCH, 1);
  tf = Ceq * Req;
  Tst3 = SIM_horowitz(nextinputtime, tf, PARM(VTHMUXDRV3), PARM(VTHOUTDRINV),
                      FALL);
...
double SIM_valid_driver_delay(int C, int A, int Ntbl, int Ntspd,
                              double inputtime) {
  double Ceq, Tst1, tf;

  Ceq = SIM_draincap(Wmuxdrv12n, NCH, 1) + SIM_draincap(Wmuxdrv12p, PCH, 1) +
        Cout;
...
double SIM_dataoutput_delay(int C, int B, int A, int Ndbl, int Nspd, int Ndwl,
  ...
  Ceq = 2 * SIM_draincap(Woutdrvnorn, NCH, 1) +
        SIM_draincap(Woutdrvnorp, PCH, 2) + SIM_gatecap(Woutdrivern, 10.0);
  tf = Ceq * SIM_transreson(Woutdrvnorp, PCH, 2);
  nordel =
      SIM_horowitz(inrisetime, tf, PARM(VTHOUTDRNOR), PARM(VTHOUTDRIVE), FALL);
  nextinputtime = nordel / (PARM(VTHOUTDRIVE));

  /* Delay of final output driver */

  Ceq =
      (SIM_draincap(Woutdrivern, NCH, 1) + SIM_draincap(Woutdriverp, PCH, 1)) *
          ((8 * B * A) / PARM(BITOUT)) +
...
double SIM_selb_delay_tag_path(double inrisetime, double *outrisetime) {
  ...
  Tst1 =
      SIM_horowitz(inrisetime, tf, PARM(VTHOUTDRINV), PARM(VTHOUTDRNAND), RISE);
...
int SIM_organizational_parameters_valid(int rows, int cols, int Ndwl, int Ndbl,
                                        int Nspd, int Ntwl, int Ntbl,
                                        int Ntspd) {
  /* don't want more than 8 subarrays for each of data/tag */

  if (Ndwl * Ndbl > PARM(MAXSUBARRAYS))
    return (0);
  if (Ntwl * Ntbl > PARM(MAXSUBARRAYS))
    return (0);
...
void SIM_calculate_time(time_result_type *result,
                        time_parameter_type *parameters) {
  int Ndwl, Ndbl, Nspd, Ntwl, Ntbl, Ntspd, rows, columns, tag_driver_size1,
      tag_driver_size2;
  double access_time;
          ...
          for (Ntwl = 1; Ntwl <= 1; Ntwl = Ntwl * 2) {
            for (Ntbl = 1; Ntbl <= PARM(MAXN); Ntbl = Ntbl * 2) {
              if (SIM_organizational_parameters_valid(
                ...
                decoder_data = SIM_decoder_delay(
                    parameters->cache_size, parameters->block_size,
                    parameters->associativity, Ndwl, Ndbl, Nspd, Ntwl, Ntbl,
                    Ntspd, &decoder_data_driver, &decoder_data_3to8,
                    &decoder_data_inv, &outrisetime);

                inrisetime = outrisetime;
                wordline_data = SIM_wordline_delay(
                    parameters->block_size, parameters->associativity, Ndwl,
                    Nspd, inrisetime, &outrisetime);

                inrisetime = outrisetime;
                bitline_data = SIM_bitline_delay(
                    parameters->cache_size, parameters->associativity,
                    parameters->block_size, Ndwl, Ndbl, Nspd, inrisetime,
                    &outrisetime);
                inrisetime = outrisetime;
                sense_amp_data = SIM_sense_amp_delay(inrisetime, &outrisetime);
                inrisetime = outrisetime;
                data_output = SIM_dataoutput_delay(
                    parameters->cache_size, parameters->block_size,
                    parameters->associativity, Ndbl, Nspd, Ndwl, inrisetime,
                    &outrisetime);
                inrisetime = outrisetime;
                ...
                decoder_tag = SIM_decoder_tag_delay(
                    parameters->cache_size, parameters->block_size,
                    parameters->associativity, Ndwl, Ndbl, Nspd, Ntwl, Ntbl,
                    Ntspd, &decoder_tag_driver, &decoder_tag_3to8,
                    &decoder_tag_inv, &outrisetime);
                inrisetime = outrisetime;

                wordline_tag = SIM_wordline_tag_delay(
                    parameters->cache_size, parameters->associativity, Ntspd,
                    Ntwl, inrisetime, &outrisetime);
                inrisetime = outrisetime;

                bitline_tag = SIM_bitline_tag_delay(
                    parameters->cache_size, parameters->associativity,
                    parameters->block_size, Ntwl, Ntbl, Ntspd, inrisetime,
                    &outrisetime);
                inrisetime = outrisetime;

                sense_amp_tag =
                    SIM_sense_amp_tag_delay(inrisetime, &outrisetime);
                inrisetime = outrisetime;

                compare_tag = SIM_compare_time(parameters->cache_size,
                                               parameters->associativity, Ntbl,
                                               Ntspd, inrisetime, &outrisetime);
                inrisetime = outrisetime;

                if (parameters->associativity == 1) {
                  mux_driver = 0;
                  valid_driver = SIM_valid_driver_delay(
                      parameters->cache_size, parameters->associativity, Ntbl,
                      Ntspd, inrisetime);
                  time_till_compare =
                      decoder_tag + wordline_tag + bitline_tag + sense_amp_tag;
                  ...
                  mux_driver = SIM_mux_driver_delay(
                      parameters->cache_size, parameters->block_size,
                  ...
                  time_till_select =
                      time_till_compare + compare_tag + mux_driver + selb;
                ...
                if (result->cycle_time +
                        1e-11 * (result->best_Ndwl + result->best_Ndbl +
                                 result->best_Nspd + result->best_Ntwl +
                                 result->best_Ntbl + result->best_Ntspd) >
                    cycle_time / PARM(FUDGEFACTOR) +
                        1e-11 * (Ndwl + Ndbl + Nspd + Ntwl + Ntbl + Ntspd)) {
                  ...
                  result->dec_tag_driver =
                      decoder_tag_driver / PARM(FUDGEFACTOR);
                  ...
                  result->dec_data_driver =
                      decoder_data_driver / PARM(FUDGEFACTOR);
                  ...
                  result->wordline_delay_data =
                      wordline_data / PARM(FUDGEFACTOR);
                  ...
                  result->sense_amp_delay_data =
                      sense_amp_data / PARM(FUDGEFACTOR);
                  result->sense_amp_delay_tag =
                      sense_amp_tag / PARM(FUDGEFACTOR);

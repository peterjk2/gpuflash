...
#define MCSCALE2 1.0 /* metal coupling capacitance scaling factor (2X) */
#define MCSCALE3 1.0 /* metal coupling capacitance scaling factor (3X) */
#define GEN_POWER_SCALE \
  (PARM(GEN_POWER_FACTOR) * PARM(GEN_POWER_FACTOR) * PARM(GEN_POWER_FACTOR))
...
#define SensePowerfactor2 \
  (PARM(Freq)) * (Vbitpre - Vbitsense) * (Vbitpre - Vbitsense)
...
/*HACK: parameters for insertion buffer for links not supported for 110nm and
 * above */

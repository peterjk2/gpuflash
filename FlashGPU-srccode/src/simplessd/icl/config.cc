...
const char NAME_WRITE_EVICT_POLICY[] = "WriteEvictPolicy";
const char NAME_WRITE_CACHE_SIZE[] = "WriteCacheSize";
const char NAME_WRITE_WAY_SIZE[] = "WriteCacheWaySize";
...
Config::Config() {
  ...
  cacheSize = 33554432;
  cacheWaySize = 1;
  writeEvictPolicy = POLICY_LEAST_RECENTLY_USED;
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_WRITE_EVICT_POLICY)) {
    writeEvictPolicy = (EVICT_POLICY)strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_WRITE_CACHE_SIZE)) {
    writeCacheSize = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_WRITE_WAY_SIZE)) {
    writeCacheWaySize = strtoul(value, nullptr, 10);
  } 
  else {
    ret = false;
  }
...
int64_t Config::readInt(uint32_t idx) {
  ...
  switch (idx) {
    case ICL_EVICT_POLICY:
      ret = evictPolicy;
      break;
    case ICL_WRITE_EVICT_POLICY:
      ret = writeEvictPolicy;
      break;            
  }
...
uint64_t Config::readUint(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case ICL_WAY_SIZE:
      ret = cacheWaySize;
      break;
    case ICL_WRITE_CACHE_SIZE:
      ret = writeCacheSize;
      break;
    case ICL_WRITE_WAY_SIZE:
      ret = writeCacheWaySize;
      break;            

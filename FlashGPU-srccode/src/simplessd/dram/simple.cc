...
void SimpleDRAM::updateDelay(uint64_t latency, uint64_t &tick) {
  if (tick > 0) {
    //printf("latency %lu tick %lu lastDRAMAccess %lu\n", latency, tick, lastDRAMAccess);
    ...
    //Jie: lastDRAMAccess is inaccurate for DRAM 
    //tick = lastDRAMAccess;
    tick += latency;
  }
}
...
void SimpleDRAM::read(void *addr, uint64_t size, uint64_t &tick) {
  ...
  uint64_t latency = (uint64_t)(
    size / interfaceBandwidth);
      //pageFetchLatency + pageCount * pStructure->pageSize / interfaceBandwidth);
  //printf("size %lu interfaceBandwidth %lf latency %lu\n", size, interfaceBandwidth, latency);
